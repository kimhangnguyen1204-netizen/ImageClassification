...
fruits_classes = [element.lower() for element in fruits_classes]
...
for filename in os.listdir(input_folder):
    if filename.endswith(('.jpg', '.JPG', '.jpeg', '.png')):  # Check if the file is an image
        ...
        elif classification in fruits_classes:
            shutil.move(img_path, os.path.join(fruits_folder, filename))
            print(f'{filename} moved to {fruits_folder}')    
        else:
